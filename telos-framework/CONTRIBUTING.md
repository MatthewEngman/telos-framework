...
- **Publish (optional):** [`.github/workflows/publish.yml`](../.github/workflows/publish.yml) is **manual** (`workflow_dispatch`). Add a repository secret **`PYPI_API_TOKEN`** (PyPI API token), then run the workflow to build and upload from `telos-framework/dist/`.
